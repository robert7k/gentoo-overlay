...
EAPI=6
inherit autotools eutils toolchain-funcs vcs-snapshot versionator
...
PATCH_NAME="${PN}-$(get_version_component_range 1-2)-chappa-all.patch.gz"
SRC_URI="http://repo.or.cz/alpine.git/snapshot/${GIT_COMMIT}.tar.gz -> ${P}.tar.gz
	chappa? ( http://alpine.x10host.com/alpine/patches/${PN}-${PATCH_LEVEL}/all.patch.gz
	-> ${PATCH_NAME} )"
RESTRICT="mirror"
...
src_prepare() {
	default
	use chappa && epatch "${DISTDIR}/${PATCH_NAME}"
	eautoreconf
}
